...
void drawinit()
{
	loadimage(&num_2[0], LPCTSTR("./images/pg1.bmp")); // 用于显示已经翻开的空格
	loadimage(&num_2[1], LPCTSTR("./images/p1.bmp"));
	loadimage(&num_2[2], LPCTSTR("./images/p2.bmp"));
	loadimage(&num_2[3], LPCTSTR("./images/p3.bmp"));
	loadimage(&num_2[4], LPCTSTR("./images/p4.bmp"));
	loadimage(&num_2[5], LPCTSTR("./images/p5.bmp"));
	loadimage(&num_2[6], LPCTSTR("./images/p6.bmp"));
	loadimage(&num_2[7], LPCTSTR("./images/p7.bmp"));
	loadimage(&num_2[8], LPCTSTR("./images/p8.bmp"));
	// 以上的为载入的显示周围雷的个数
	loadimage(&bomb_2_1, LPCTSTR("./images/bomb.bmp"));
	loadimage(&bomb_2_2, LPCTSTR("./images/redbomb.bmp"));
	loadimage(&Score, LPCTSTR("./images/r1.bmp"));
	// 以上为点击爆炸后显示的雷及相关的图片
	loadimage(&flag_2_1, LPCTSTR("./images/flag.bmp"));
	loadimage(&flag_2_2, LPCTSTR("./images/flag2.bmp"));
	// 以上为右击的问号以及旗帜标识
	loadimage(&masks_2[0], LPCTSTR("./images/t0.bmp"));
	loadimage(&masks_2[1], LPCTSTR("./images/t1.bmp"));
	loadimage(&masks_2[2], LPCTSTR("./images/t2.bmp"));
	loadimage(&masks_2[3], LPCTSTR("./images/t3.bmp"));
	loadimage(&masks_2[4], LPCTSTR("./images/t4.bmp"));
	loadimage(&masks_2[5], LPCTSTR("./images/t5.bmp"));
	loadimage(&masks_2[6], LPCTSTR("./images/t6.bmp"));
	loadimage(&masks_2[7], LPCTSTR("./images/t7.bmp"));
	loadimage(&masks_2[8], LPCTSTR("./images/t8.bmp"));
	loadimage(&masks_2[9], LPCTSTR("./images/t9.bmp"));
	loadimage(&dot, LPCTSTR("./images/dot.bmp"));
	// 以上为时钟的显示
	loadimage(&StateSetUp, LPCTSTR("./images/r2.bmp"));
	loadimage(&StateVictory, LPCTSTR("./images/r3.bmp"));
	loadimage(&StateFailure, LPCTSTR("./images/r4.bmp"));
	// 以上为状态栏的图片
	loadimage(&Background, LPCTSTR("./images/bg.bmp")); // 背景图
	return;
}
	...
	if (m.uMsg == WM_LBUTTONDOWN) // 左击鼠标判断
	{
		...
		if (mask[y][x] != 2) // 在允许显示的情况下进行操作
		{
			...
			if (map[y][x] == 10) // 如果点中雷判断
			{
				...
				mark = 1; // 强制结束
				// 以下为爆炸后的声音
				mciSendString(LPCWSTR("stop fmusic"), NULL, 0, NULL);						  // 先把前面一次的音乐停止
				mciSendString(LPCWSTR("close fmusic"), NULL, 0, NULL);						  // 先把前面一次的音乐关闭
				mciSendString(LPCWSTR("open .\\audio\\fail.mp3 alias fmusic"), NULL, 0, NULL); // 打开失败音乐
				mciSendString(LPCWSTR("play fmusic"), NULL, 0, NULL);						  // 仅播放一次
			}
		}
		// 以下为点击鼠标的声音播放
		mciSendString(LPCWSTR("stop cmusic"), NULL, 0, NULL);						   // 先把前面一次的音乐停止
		mciSendString(LPCWSTR("close cmusic"), NULL, 0, NULL);						   // 先把前面一次的音乐关闭
		mciSendString(LPCWSTR("open .\\audio\\click.wav alias cmusic"), NULL, 0, NULL); // 打开跳动音乐
		mciSendString(LPCWSTR("play cmusic"), NULL, 0, NULL);						   // 仅播放一次
	}
...
void Judge()
{
	...
	if (cnt == 4 * NBomb) // 胜利结束游戏，并播放音乐
	{
		mark = 1;
		jud = 1;
		mciSendString(LPCWSTR("stop wmusic"), NULL, 0, NULL);						 // 先把前面一次的音乐停止
		mciSendString(LPCWSTR("close wmusic"), NULL, 0, NULL);						 // 先把前面一次的音乐关闭
		mciSendString(LPCWSTR("open .\\audio\\win.mp3 alias wmusic"), NULL, 0, NULL); // 打开胜利音乐
		mciSendString(LPCWSTR("play wmusic"), NULL, 0, NULL);						 // 仅播放一次
	}
}
